...
def plot_episode_joint_comparison(gt_arr, pred_arr, ep_name, ep_avg_mse, plots_dir):
    """Plot per-dimension GT and prediction curves for one episode.
    
    Args:
        gt_arr: (num_windows, num_open_loop_steps, action_dim) or (num_steps, action_dim)
        pred_arr: same shape as gt_arr
    """
    # Flatten windows into a single sequence if needed
    if gt_arr.ndim == 3:
        num_windows, num_steps, action_dim = gt_arr.shape
        gt_arr_flat = gt_arr.reshape(num_windows * num_steps, action_dim)
        pred_arr_flat = pred_arr.reshape(num_windows * num_steps, action_dim)
    else:
        gt_arr_flat = gt_arr
        pred_arr_flat = pred_arr

    dim = gt_arr_flat.shape[1]
    ...
    for d in range(dim):
        axes[d].plot(gt_arr_flat[:, d], label="GT", color="blue")
        axes[d].plot(pred_arr_flat[:, d], label="Pred", color="red", linestyle="--")
        axes[d].set_ylabel(dim_names[d] if d < len(dim_names) else f"Dim {d}")
        axes[d].grid(True)
        axes[d].legend()

    axes[-1].set_xlabel("Step")
    plt.suptitle(f"Episode {ep_name} - Joint Action Comparison (MSE={ep_avg_mse:.4f})")
    plt.tight_layout()

    out_file = plots_dir / f"episode_{ep_name}_joint_comparison.png"
...
def eval_openloop_joint(cfg: JointEvalConfig) -> None:
    ...
    for ep_idx, ep_dir in enumerate(tqdm.tqdm(episode_dirs, desc="Episodes")):
        ...
        # Process open-loop windows: every num_open_loop_steps, collect a window of GT actions
        for step_dir in step_dirs[::cfg.num_open_loop_steps]:
            ...
            # Collect GT actions for the window
            for i in range(cfg.num_open_loop_steps):
                new_data_h5 = step_dir.parent / f"{start_num + i:04d}" / "data.h5"
                if not new_data_h5.exists():
                    if gt_action_window:
                        gt_action_window.append(gt_action_window[-1])
                        continue
                    logger.warning("Missing first action file at %s, skip this window.", new_data_h5)
                    gt_action_window = []
                    break

                with h5py.File(new_data_h5, "r") as fp:
                    if cfg.action_key not in fp:
                        raise KeyError(f"Missing action key '{cfg.action_key}' in {new_data_h5}")
                    action = np.asarray(fp[cfg.action_key][()], dtype=np.float32)
                    gt_action_window.append(action)

            if not gt_action_window:
                continue

            # Load task label and proprio from the first step of the window
            ...
            try:
                ...
                gt_window_array = np.stack(gt_action_window)
                pred_action = _align_action_shape(pred_action, gt_window_array)
            except Exception as exc:
                logger.error("Prediction failed at %s: %s", step_dir, exc)
                break

            gt_actions.append(gt_window_array)
            pred_actions.append(pred_action)

            # Store step-level pred/GT pairs for each step in the window
            for i, (gt_step, pred_step) in enumerate(zip(gt_window_array, pred_action)):
                step_idx = start_num + i
                step_name = f"step_{step_idx:04d}"
                ep_step_pairs[step_name] = {
                    "pred_value": pred_step.tolist(),
                    "GT_value": gt_step.tolist(),
                }

            mse = float(np.mean((pred_action - gt_window_array) ** 2))
            ep_mse_sum += mse
            ep_steps += 1
        ...
        np.savez_compressed(
            plots_dir / f"episode_{ep_dir.name}_joint_predictions.npz",
            gt_actions=gt_arr,
            pred_actions=pred_arr,
        )

        if ep_idx < cfg.plot_first_n_episodes:
            plot_episode_joint_comparison(gt_arr, pred_arr, ep_dir.name, ep_avg_mse, plots_dir)
