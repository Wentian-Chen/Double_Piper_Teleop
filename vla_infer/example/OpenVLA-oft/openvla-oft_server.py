"""Launch a ZMQ-based Openvla-OFT inference server."""
...
from vla_infer.src.models.OpenVLA_OFT_model import VlaAdapterModel

draccus_wrap = t.cast(t.Callable[..., t.Callable[..., t.Any]], getattr(draccus, "wrap"))

@dataclass
class VlaAdapterServerConfig:
    """Runtime config for launching OpenVLA-oft inference server."""
    # OpenVLA-oft checkpoint directory.
    model_path: str = ""
...
def main(cfg: VlaAdapterServerConfig) -> None:
    ...
    logging.info("Starting OpenVLA-oft server on tcp://%s:%s", cfg.ip, cfg.port)
